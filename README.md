...
> 네이버 스포츠의 공개 API를 사용한다. 요청은 초당 2.5회로 제한해 두었다.
> 개인 학습 목적이며 수집한 데이터는 저작권이 네이버와 KBO에 있으므로
> 이 저장소에는 코드만 올린다.
...
진행 중인 경기를 30초마다 갱신하며 승률과 근거를 보여준다.

```bash
streamlit run app/dashboard.py
```

![대시보드 전체](reports/screenshots/dashboard-01-overview.png)

회별 승률 추이. 가로축이 이벤트 번호가 아니라 회(초/말)라 야구적으로 읽힌다.
9회말 6점을 내며 2%에서 97%로 뒤집은 경기다.

![회별 추이](reports/screenshots/dashboard-02-track.png)

각 반이닝의 승률 변화와 그 이닝을 규정한 타석.
9회말 `김재현 : 좌익수 뒤 홈런 (+91.4%p)` 처럼 타석 단위로 묶어서 보여준다.

![근거와 만약에](reports/screenshots/dashboard-03-explain.png)

왼쪽은 지금 승률을 만든 요인, 오른쪽은 상황을 하나만 바꿔 다시 계산한 값이다.
아래는 다음 타석 결과별 승률.
...
**점수차만 쓰는 단순 모델을 이겼다.** Cross Validation(908경기)에서도 0.4581 대 0.4548로
같은 방향이 나왔다. 두 평가 방식이 일치하니 분할 운은 아니다.

### 네이버와 비교 — 아직 한쪽으로 기울지 않은 73,890줄

| 모델 | log loss | Brier | AUC |
|---|---|---|---|
| 네이버 자체 승률 | **0.4279** | **0.1426** | **0.879** |
| 이 모델 | 0.4327 | 0.1452 | 0.872 |

**네이버 실서비스 승률에는 졌다.** Cross Validation에서도 0.4559 대 0.4670으로 뒤진다.

표를 따로 둔 이유는 **채점한 줄이 다르기 때문**이다. 위 표는 76,802줄, 아래 표는 73,890줄이다.
두 표의 숫자를 가로질러 비교하면 안 된다.

네이버 승률은 `0.0`으로 찍히는 줄이 있는데, 값이 없는 경우와 실제로 0%인 경우가 섞여 있다.
원정 승률을 같이 봐야 구분되지만(둘을 더해 100이면 진짜 값, 0이면 결측)
지금 데이터에는 홈 값만 저장돼 있다. 그래서 `0.0`과 `100.0`을 모두 뺐다.
양 끝을 대칭으로 빼므로 어느 팀에게도 유리하지 않다.
...
| 평균 점수차 | 6.17점 | 2.44점 |
| 평균 이닝 | 8.4회 | 5.0회 |
| 예측이 5% 미만 또는 95% 초과 | 80% | 23% |

8회에 6점 차. 이미 끝난 경기다. 이런 줄만 따로 채점하면 이 모델의 log loss가 0.09다.
쉬운 문제가 빠지니 두 표의 점수 차이(0.4192 → 0.4327)가 생긴다.
모델이 나빠진 게 아니라 시험이 어려워진 것이다.
...
경기 중 어떤 상황이 주어졌을 때, 홈팀이 최종적으로 이길 확률은 얼마인가.
...
| 주자와 카운트 | 주자 상황(8가지), 기대득점, 잔여 득점 잠재력, 아웃, 볼, 스트라이크, 끝내기 기회, 홈팀 마지막 공격 |
...
**기대득점(RE).** 주자와 아웃 조합 24가지 각각에서 앞으로 몇 점이 더 날지의 평균값이다.
주자 상황 8가지(각 베이스에 주자가 있냐 없냐의 조합) × 아웃 3가지 = 24개다.
1,136경기에서 직접 계산했고, 학습에 쓴 경기만 쓴다.

| 주자 | 0아웃 | 1아웃 | 2아웃 |
|---|---|---|---|
| 주자 없음 | 0.562 | 0.309 | 0.121 |
| 1루 | 1.000 | 0.632 | 0.285 |
| 2루 | 1.259 | 0.834 | 0.423 |
| 1·2루 | 1.611 | 1.160 | 0.541 |
| 3루 | 1.527 | 1.139 | 0.472 |
| 1·3루 | 1.927 | 1.277 | 0.599 |
| 2·3루 | 2.317 | 1.701 | 0.702 |
| 만루 | 2.434 | 1.866 | 0.889 |

2아웃 만루(0.889)가 무사 1루(1.000)보다 낮다. 아웃 두 개가 주자 셋보다 크다.
...
부스팅 라운드 수와 확률 보정 방식은 학습 경기 안에서 5-Fold Cross Validation으로 정한다.
...
| Cross Validation 320경기 | 0.4552 (1위) | 0.4948 | 0.4917 |
...
### 만루 홈런의 대표 플레이가 '주자 홈인'으로 잡혔다

9회말 만루 홈런으로 뒤집은 경기인데, 그 이닝의 대표 플레이가
`3루주자 추재현 : 홈인`으로 나왔다. 홈런이 아니라.

중계 데이터에서 만루 홈런은 네 줄로 쪼개진다. 홈런 한 줄, 주자 홈인 세 줄.
그리고 **점수가 실제로 올라가는 건 홈인 줄**이다.
WPA를 이벤트 단위 차분으로 계산하면 이렇게 된다.

| 이벤트 | 스코어 | WPA |
|---|---|---|
| 김재현 : 좌익수 뒤 홈런 | 7-5 | +4.2%p |
| 1루주자 안치홍 : 홈인 | 7-6 | **−2.2%p** |
| 2루주자 데이비슨 : 홈인 | 7-7 | +43.7%p |
| 3루주자 추재현 : 홈인 | 7-8 | +45.7%p |

`1루주자 홈인`이 음수인 게 눈에 띈다. **1점을 얻었는데 승률이 떨어졌다.**
같은 줄에서 주자 상황이 만루에서 주자 없음으로 바뀌기 때문이다.
모델은 "1점 얻고 만루가 사라짐"으로 읽는다. 홈런은 원래 주자를 다 불러들이는
플레이인데, 이벤트가 쪼개지면서 득점과 주자 소멸이 다른 줄로 갈라진 것이다.

**대응** — WPA를 타석 단위로 바꿨다. 사실 이게 원래 정의다.
야구에서 쓰는 WPA는 FanGraphs든 Baseball-Reference든
"타석 시작 승률 → 타석 종료 승률"로 계산한다. 이벤트 단위 차분이 비표준이었다.

타석으로 묶으면 위 네 줄이 하나가 된다. `김재현 : 좌익수 뒤 홈런 +91.4%p`.

바꾸고 나니 승부처 목록이 전부 실제 타격 결과로 바뀌었다.
이전에는 `서건창 : 삼진 아웃`, `권혁빈 : 2루수 플라이 아웃` 같은 게
상위에 올라와 있었는데, 지금은 홈런·3루타·병살타가 잡힌다.
승률 곡선 자체는 투구 단위로 그대로 둔다. 묶는 건 플레이를 고를 때만이다.
...
### WPA — 그 타석이 승률을 얼마나 바꿨나

**타석 시작 승률과 타석 종료 승률의 차이**다. 안타 하나로 40%에서 55%가 됐다면 +15%p.
야구계에서 이미 쓰는 개념이라 따로 설명할 필요가 없고, 모델과 무관하게 정의된다.

타석 단위인 게 중요하다. 이벤트 단위로 계산하면 만루 홈런이 네 조각으로 흩어져
대표 플레이가 '주자 홈인'으로 잡힌다(Trouble Shooting 참고).
승률 곡선 자체는 투구 단위로 그리고, 묶는 것은 플레이를 고를 때만이다.

### SHAP — 지금 이 승률을 만든 요인

기준 승률에서 출발해 각 항목이 몇 %p씩 밀어올렸는지 나눈다.
log odds 같은 단위 대신 %p로 바꿔서, 다 더하면 현재 승률이 되게 했다.
...
src/features.py         쓰는 정보 계산, 기대득점, 상황 정규화
